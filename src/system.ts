...
const sequenceAsyncFns = <TFn extends () => Promise<void>>(
  asyncFns: TFn[],
): Promise<void> => {
  return asyncFns.reduce(
    (promise, fn) => promise.then((_result) => fn()),
    Promise.resolve(),
  );
};

const executeMethodSequentially = (
...
const startAllComponents = (components: Component[]) =>
  executeMethodSequentially(components, 'start');
const stopAllComponents = (components: Component[]) =>
  executeMethodSequentially(components, 'stop');
...
export const system = (components: ComponentMapping): System => {
  ...
  class SystemComponent extends Component {
    start() {
      return sequenceAsyncFns(
        this.startOrder.map((level) => () => startAllComponents(level)),
      );
    }
